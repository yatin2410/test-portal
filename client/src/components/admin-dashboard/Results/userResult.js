...
class UserResult extends Component {
  ...
  onView(id) {
    this.openInNewTab(window.location.origin.toString()+"/viewquizadmin/"+id+"/"+this.props.match.params.id);
  }
  render() {
    return (
      <div>
        {
          ...
          <div className="mt-5">
          <h6 ><Link to="/dashboard/users" className="arrowFont"><i class="fas fa-arrow-left arrowFont" style={{fontSize:"0.75em"}}></i>  Back to Users Page </Link></h6>
            <h4 className="row justify-content-md-center">{this.state.userName+"'s Quizzes"}</h4>
            <SearchTable
