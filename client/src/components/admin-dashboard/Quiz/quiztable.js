...
function Table(props) {
  ...
  let reverseList = isSortReverse
    ...
    : SORTS[sortKey](list);
  reverseList = reverseList.filter(
    item =>
      item.name.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1 ||
      item.startDate.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1 ||
      item.endDate.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1 ||
      item.duration === Number(searchTerm) ||
      item.perToPass === Number(searchTerm) ||
      item.groups.indexOf(searchTerm) !== -1
  );
  return (
      ...
      <table className="table table-bordered table-hover table-striped modifiedtable">
        <thead>
          <tr>
            ...
            <th style={{ width: "15%" }}>
              <Sort sortKey={"ENDDATE"} onSort={onSort} activeSortKey={sortKey}>
                End Date
              </Sort>{" "}
            </th>
            <th style={{ width: "12%" }}>
              <Sort
                ...
                onSort={onSort}
                activeSortKey={sortKey}>
                <span style={{ fontSize: "0.85em" }}>Duration</span>
              </Sort>{" "}
            </th>
            <th style={{ width: "12%" }}>
              <Sort
            ...
            <th style={{ width: "12%" }}>
              <Sort sortKey={"GROUPS"} onSort={onSort} activeSortKey={sortKey}>
                Groups
              </Sort>{" "}
            </th>
            <th style={{ width: "8%" }}>
              <span style={{ fontSize: "0.8em" }}>Questions</span>
            </th>
            ...
            <tr key={item._id}>
              ...
              <td>
                {item.groups.map(item => (
                  <span className="row ml-1">{item}</span>
                ))}
              </td>
              <td>
                <button
                  className="btn"
                  onClick={() => onOpenQuestions(item._id)}>
        ...
        <div className="row mt-4 justify-content-end mr-3">
          <div className="col-3">
            <button
              ...
              onClick={() => this.props.history.push("/dashboard/addquiz")}>
              Add Quiz
            </button>
          </div>
