...
export default class Timmer extends Component {
    ...
    componentWillReceiveProps(nextProps) {
      if (nextProps.Time) {
        this.setState({ time: nextProps.Time, checkpoints: this.countCheckPoint(nextProps.Time) });
      }
    }
    ...
    render() {
      ...
      const checkpoints = [{
          time: 1000 * 60 * 4.5,
          callback: () => this.props.putFlashMsg({msg:"Only 5 minutes are remaining",type:"alert-danger"})
        },
